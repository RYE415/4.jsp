...
public class BoardDAO {//MemberDAO
	...
	/////1.검색어에 해당되는 총 레코드 수를 구하는 메서드(검색분야,검색어)
	...
	public Hashtable pageList(String pageNum,int count) {
		//1.페이징 처리 결과를 저장할 Hashtabel 객체를 선언
		 ...
		 //게시판을 맨 처음 실행시키면 무조건 1페이지부터 출력->가장 최근의 글이 나오기때문에 
			if(pageNum==null){
			  ...
			  //총 페이지 수, 시작, 종료 페이지 계산->list.jsp에서 이미 코딩
			  //->모델1에서의 list.jsp에서 복사해온다.
			  //1.총페이지 구하기 122/10=12.2+1.0=13(122%10)
				int pageCount = count / pageSize + (count % pageSize == 0 ? 0 : 1);
				// 2.시작페이지
				int startPage = 0;
				if (currentPage % blockSize != 0) {// 1~9,11~19,21~29(10의 배수가 아닌경우)
					startPage = currentPage / blockSize * blockSize + 1;// 경계선때문에
				...
				//페이징 처리에 대한 계산 결과=>Hashtable,HashMap
