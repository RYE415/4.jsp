...
public class BoardDAO {
	...
	private DBConnectionMgr pool=null;//1.객체 생성할 멤버변수 선언
	//공통
	...
	//3.메서드 작성(페이징 처리를 위한 메서드)=총 레코드 수(=총 게시물 수=총 회원 수)
	...
	//2.글 목록 보기에 대한 메서드 구현->레코드 한 개 이상-> 한 페이지당 10개씩 담기
	...
	public List<BoardDTO>getArticles(int start,int end){
		...
		try {
			con=pool.getConnection();
			/*
			 * 그룹번호가 가장 최신의 글을 중심으로 정렬하되, 만약에 level이 같은 경우
			 * step값으로 오름차순으로 통해서 몇번째 레코드 번호를 기준해서 몇개까지
			 * 정렬할 것인가를 지정해주는 SQL 구문
			 */
			sql="select * from board order by ref desc,re_step limit ?,?";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1,start-1);//mysql은 레코드순번이 내부적으로 0 시작
			pstmt.setInt(2,end);//불러와서 담을 갯수(ex 10)
			rs=pstmt.executeQuery();
			if(rs.next()) {//보여주는 결과가 있다면
				//10->end갯수만큼 데이터 담을 공간 생성
				...
				do {
					...
					article.setWriter(rs.getString("writer"));//부적합한 열입니다.
					...
					article.setRe_step(rs.getInt("re_step"));//답변글 나오는 순서
					article.setRe_level(rs.getInt("re_level"));//들여쓰기 유무
					article.setContent(rs.getString("content"));
					article.setIp(rs.getString("ip"));//접속자
					*/
					//추가
					articleList.add(article);//생략하면 데이터가 저장X-> for문 시 에러유발
	...
	public void insertArticle(BoardDTO article) {//~(MemberDTO mem)
		...
		int num=article.getNum();//0:신규글 <-> 0이 아닌 경우(양수)
		...
		try {
			...
			if(rs.next()) {//데이터가 있다면
				...
				number=1;//테이블에 한개의 데이터가 없다면 최초 1 부여
			}
			//답변글이라면 -> 0이 아니라면 답변글임!
			if(num!=0) {
				//같은 그룹번호를 가지고 있으면서 나보다 step 값이 큰 게시물을 찾아서 step 하나 증가
			...
			System.out.println("게시판의 글쓰기 성공 유무(insert)=>"+insert);//1 or 0
	...
	//public MemberDTO get Member(String id){~}
	...
	//접근지정자가 private가 되는 경우=>외부에서 호출X 클래스 사용O
		...
		try {
			...
			if (rs.next()) {// 데이터가 있다면
				...
				if (dbpasswd.equals(article.getPasswd())) {
					...
					int update = pstmt.executeUpdate();
					System.out.println("게시판의 글 수정 성공 유무(update)=>" + update);// 1 or 0
					x = 1;// 글 수정 성공
				} else {
					x = 0;// 수정실패
	...
	public int deleteArticle(int num,String passwd) {
		...
		try {
			...
			if (rs.next()) {// 데이터가 있다면
				...
				System.out.println("dbpasswd=>" + dbpasswd);
				/*
				 * 암호가 틀리다고 자꾸 나오는 경우-> num가 제대로 전달?
				...
				if (dbpasswd.equals(passwd)) {
					...
					System.out.println("게시판의 글 삭제 성공 유무(delete)=>" + delete);// 1 or 0
					x = 1;// 글 삭제 성공
				} else {
					x = 0;// 삭제실패
